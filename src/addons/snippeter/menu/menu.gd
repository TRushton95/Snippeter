...
@onready var _editor : Snippeter_Editor = $Editor
...
func _on_listing_copy_button_pressed(snippet_name: String) -> void:
	var snippet_content = Snippeter_Disk.read_snippet_data(snippet_name)
...
func _on_listing_edit_button_pressed(snippet_name: String) -> void:
	var snippet_content = Snippeter_Disk.read_snippet_data(snippet_name)
	var snippet = Snippeter_Snippet.new(snippet_name, snippet_content)
...
func _on_editor_save_button_pressed(snippet: Snippeter_Snippet) -> void:
	if !_editor.is_edit_mode() && Snippeter_Disk.snippet_exists(snippet.get_name()):
		_editor.show_existing_name_dialog()
		return
	
	Snippeter_Disk.save_snippet(snippet.get_data(), snippet.get_name())
...
func _on_delete_confirmation_dialog_confirmed() -> void:
	...
	Snippeter_Disk.delete(_pending_snippet_deletion)
...
func _ready() -> void:
	...
	for snippet in Snippeter_Disk.read_all_snippets() as Array[Snippeter_Snippet]:
		add_listing(snippet.get_name())
...
func add_listing(snippet_name: String) -> void:
	...
	var listing : Snippeter_Listing = listing_scene.instantiate()
